...
def compare_arps(arp_file_today, arp_file_yesterday, switch_name):

    if arp_file_today != 'current':
        try:
            with open(arp_file_today, 'r') as f_today, open(arp_file_yesterday, 'r') as f_yesterday:
        ...
        try:
            with open(arp_file_yesterday, 'r') as f_yesterday:
                    yesterday_lines = f_yesterday.readlines() 
        except FileNotFoundError:
            return None, 10

        f_today = get_curr_arps(switch_name, get_info())
...
web_compare_arp = Blueprint('web_compare_arp', __name__)
@web_compare_arp.route('/compare_arp', methods=['GET'])
@cross_origin()
def compare_arp_main():
    ...
    if status == 12 and formatted_diff != None:
        ...
        #case where one of the arps isn't found
        return jsonify([[["none","Arps not found error"]]])
    else: 
        print("error")
