...
const App = () => {
    ...
    useEffect(() => {
        ...
        console.log(isValid, date1, date2)
    ...
    const getConfigs = () => {
        ...
        const newUrl = `/api/get_config?switch_name=${encodeURIComponent(switchName)}&date=${encodeURIComponent(convertToMMDDYYYY(date2))}`;
        fetch(newUrl)
            .then(response => response.json())
            .then(config => {
        ...
        const oldUrl = `/api/get_config?switch_name=${encodeURIComponent(switchName)}&date=${encodeURIComponent(convertToMMDDYYYY(date1))}`;
        fetch(oldUrl)
            .then(response => response.json())
  ...
  const compareConfigs = () => {
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            <h1 className="text-2xl font-bold text-center border-b-2 border-blue-500 pb-2 mb-4">Config Checker</h1>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <button
                    ...
                    className={`p-3 rounded font-bold ${isGetConfigsDisabled ? 'bg-gray-400' : 'bg-green-500 text-white hover:bg-green-600 cursor-pointer'}`}
                >
                    Get Configs
                </button>
                <button
                    ...
                    className={`p-3 rounded font-bold ${isCompareConfigsDisabled ? 'bg-gray-400' : 'bg-blue-500 text-white hover:bg-blue-600 cursor-pointer'}`}
                >
                    Compare Configs
                </button>
            </div>
            <div className="grid grid-cols-3 gap-4 mb-4 h-screen w-full">
                <div className="mb-4">
                    <h3 className="text-lg font-bold">Old Config:</h3>
                    <pre className="resize-y p-2 bg-gray-100 border rounded overflow-auto h-32">{configDisplay1}</pre>
                </div>

                <div>
                    <h3 className="text-lg font-bold">Comparison:</h3>
                    <pre className="resize-y p-2 bg-gray-100 border rounded overflow-auto h-32">{compareDisplay}</pre>
                </div>

                <div className="mb-4">
                    <h3 className="text-lg font-bold">New Config:</h3>
                    <pre className="resize-y p-2 bg-gray-100 border rounded overflow-auto h-32">{configDisplay2}</pre>
                </div>
